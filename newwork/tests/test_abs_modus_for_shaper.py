...
class Slave(threading.Thread):
    ...
    def __init__(self, ):
        ...
        self.RUNNING = True
        self.data_buffer = ""
        self.crc16 = crcmod.predefined.mkCrcFun('modbus')#设置‘modbus’为校验码
        self.direction = 30000
        self.get_direction = 0
        self.midpoint = 25000#random.randint(30, 80)
        logger.warning("dist = " + str(self.midpoint))#日志信息的重要程度为warning
    ...
    def emit_sharp(self):
        while self.RUNNING:
            time.sleep(1)
            self.sharp_return.emit(self.get_sharp_direction())#把日志输出到文件
